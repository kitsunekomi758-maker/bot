...
async def on_message_edit(before, after):
    # This ignores edits made by the bot itself
    if before.author.bot:
        return

    print(f"User {before.author} edited a message!")
...
async def on_raw_message_delete(payload):
    # This works even if the message isn't in the bot's memory
    channel = bot.get_channel(payload.channel_id)
    log_channel = bot.get_channel(LOG_CHANNEL_ID)

    # If the message was in the cache, we get the content
    content = payload.cached_message.content if payload.cached_message else "Message too old (not in cache)"
    author = payload.cached_message.author if payload.cached_message else "Unknown User"
